...
public static class Minerals_World_NaturalRockTypesIn_Patch
{
    ...
    {
        string[] volcanicBiomes = new string[] { "AB_PyroclasticConflagration", "LavaField" };
        if (__instance.grid.Tiles.ToList()[tile].Biomes.Any(b => volcanicBiomes.Contains(b.defName)))
        {
            List<ThingDef> replacedList = new List<ThingDef>();
            replacedList.Add(DefDatabase<ThingDef>.GetNamed("AB_Obsidianstone"));
        ...
        else
        {
            ...
            int num = Rand.RangeInclusive(MineralsFrameworkMain.Settings.terrainCountRangeSetting.min, MineralsFrameworkMain.Settings.terrainCountRangeSetting.max);
            if (num > list.Count)
            {
                num = list.Count;
            }
