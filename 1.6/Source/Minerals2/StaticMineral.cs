...
//        }
        ...
        public virtual void incPctYeild(float amount, Pawn miner)
        {
            ...
            foreach (RandomResourceDrop toDrop in attributes.randomlyDropResources)
            {
                ...
                float dropChance = size * toDrop.DropProbability * Minerals2Main.Settings.resourceDropFreqSetting;
                ...
                // Drop resource
                int dropNum = (int)Math.Round(toDrop.CountPerDrop * Minerals2Main.Settings.resourceDropAmountSetting * dropChance);
                if (dropNum >= 1)
                {
        ...
        {
           return attributes.mineSpeedFactor * Minerals2Main.Settings.miningEffortSetting / size;
        }
...
//        }
        ...
        public virtual float submersibleFactor()
        {
            ...
            if (!Minerals2Main.Settings.underwaterMineralsSetting)
            {
                return 1f;
            }
        ...
        public virtual void initializeTextureLocations()
        {
            ...
            for (int i = 0; i < textureLocations.Length; i++)
            {
                ...
                pos.x += randPos(attributes.visualClustering, attributes.visualSpread * Minerals2Main.Settings.visualSpreadFactor);
                pos.z += randPos(attributes.visualClustering, attributes.visualSpread * Minerals2Main.Settings.visualSpreadFactor);
...
//            float xPropDistToEven = Math.Abs(1f - ((getTextureLocation(i).x + 0.5f) % 2f));
        ...
        {
            if (Minerals2Main.Settings.mineralsGrowUpWallsSetting && attributes.growsUpWalls)
            {
        ...
        public virtual string ResourceDropList()
        {
            ...
            foreach (RandomResourceDrop resource in attributes.randomlyDropResources)
            {
                ...
                float meanDrop = resource.CountPerDrop * resource.DropProbability * size * Minerals2Main.Settings.resourceDropAmountSetting * Minerals2Main.Settings.resourceDropFreqSetting;
                if (meanDrop < 0.01f) 
                {
                    continue;
                }
    ...
    public class ThingDef_StaticMineral : ThingDef
    {
        ...
        public virtual float abundanceSettingFactor()
        {
            ...
            {
                factor = factor * Minerals2Main.Settings.crystalAbundanceSetting;
            }
            if (tags.Contains("boulder"))
            {
                factor = factor * Minerals2Main.Settings.boulderAbundanceSetting;
            }
            if (tags.Contains("small_rock"))
            {
                factor = factor * Minerals2Main.Settings.rocksAbundanceSetting;
            }
            if (tags.Contains("wall") && Minerals2Main.Settings.replaceWallsSetting == false)
            {
                factor = 0f;
            }
            if (tags.Contains("fictional") && Minerals2Main.Settings.includeFictionalSetting == false)
            {
                factor = 0f;
            }
            return factor;
        ...
        public virtual float diversitySettingFactor()
        {
            ...
            if (tags.Contains("crystal"))
            {
                factor = factor * Minerals2Main.Settings.crystalDiversitySetting;
            }
            return factor;
        }
        ...
        public virtual bool allowReplaceSetting()
        {
            ...
            if (tags.Contains("chunk_replacer") && Minerals2Main.Settings.replaceChunksSetting == false)
            {
                output = false;
            }
            return output;
