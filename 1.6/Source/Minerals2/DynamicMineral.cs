    ...
    public class DynamicMineral : StaticMineral
    {
        ...
        public virtual float GrowthRate
        {
            get
            {
                ...
                return output * Minerals2Main.Settings.mineralGrowthSetting;
            }
        }
        ...
        public override void TickLong()
        {
            ...
            // Try to reproduce
            if (GrowthThisTick > 0 && size > attributes.minReproductionSize && Rand.Range(0f, 1f) < attributes.reproduceProp * GrowthRate * Minerals2Main.Settings.mineralReproductionSetting)
            {
    ...
    public class DynamicMineralWatcher : MapComponent
    {
        ...
        public void SpawnDynamicMinerals() 
        {
            foreach (ThingDef_DynamicMineral mineralType in DefDatabase<ThingDef_DynamicMineral>.AllDefs)
            {
                ...
                float numToCheck = map.Area * mineralType.spawnProb * perMapGrowthFactor * Minerals2Main.Settings.mineralSpawningSetting;
                if (numToCheck <= 0)
                {
                    continue;
                }
